...
(defn extract-numbers [grid]
  (mapcat (fn [row row-index]
            (let [matcher (re-matcher #"\d+" row)]
              (loop [matches []]
                (if (.find matcher)
                  (recur (conj matches {:value       (Integer/parseInt (.group matcher))
                                        :row         row-index
                                        :start-index (.start matcher)
                                        :end-index   (+ (.start matcher) (dec (count (.group matcher))))}))
                  matches))))
          grid
          (range)))

(defn extract-symbols [grid]
  (mapcat (fn [row index]
...
(defn in-same-row? [num-row ast-row num-end ast-index num-start]
  (and (= num-row ast-row)
       (or (= num-end (dec ast-index))
           (= num-start (inc ast-index)))))

(defn directly-above-below? [num-row ast-row num-start ast-index num-end]
  (and (or (= ast-row (dec num-row))
           (= ast-row (inc num-row)))
       (<= num-start ast-index num-end)))

(defn diagonally-adjacent? [num-row ast-row num-start ast-index num-end]
  (and (or (= ast-row (dec num-row))
           (= ast-row (inc num-row)))
       (or (= ast-index (dec num-start))
           (= ast-index (inc num-end)))))

(defn is-adjacent? [number symbol]
...
(defn get-adjacent-numbers [symbol numbers]
  (filter #(is-adjacent? % symbol) numbers))

(defn is-part-number? [number symbols]
  (some #(is-adjacent? number %) symbols))
...
(defn calculate-ratio-for-asterisk [asterisk numbers]
