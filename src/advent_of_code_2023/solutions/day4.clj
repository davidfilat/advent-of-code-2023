...
(defn process-card [card-stack card-index]
  (let [card        (nth card-stack card-index)
    ...
    (if (zero? card-score) card-stack (map (fn [curr-card]
                                             (if (and (> (:card-number curr-card) card-number) (<= (:card-number curr-card) (+ card-number card-score)))
                                               (update curr-card :copies #(+ card-copies %))
                                               curr-card))
                                           card-stack))))

(defn process-card-stack [card-stack]
