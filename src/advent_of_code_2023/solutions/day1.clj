...
(defn keep-only-integers-from-string
  [s]
  (let [split-string (str/split s #"[^0-9]")
        non-blank-seq (filter #(not (str/blank? %)) split-string)]
    (str/join "" non-blank-seq)))

(defn merge-numbers [s] (Integer/parseInt (str/join "" s)))

(defn get-head-and-last [s] [(first s) (last s)])

(defn replace-words-with-digits-sequential
  [sentence]
  (let [digit-map {"one" "1",
                   "two" "2",
                   "three" "3",
